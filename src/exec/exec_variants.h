...
/**
 * Hook for the `execl()`, `execle()` and `execlp()` functions in `unistd.h`
 * which redirects the call to `execve()`.
 */
int execl_hook(bool hook, int variant, const char *name, const char *argv0, va_list ap);

/**
 * Hook for the `execv()` function in `unistd.h` which redirects the call to `execve()`.
 */
int execv_hook(bool hook, const char *name, char *const *argv);

/**
 * Hook for the `execvp()` function in `unistd.h` which redirects the call to `execve()`.
 */
int execvp_hook(bool hook, const char *name, char *const *argv);

/**
 * Hook for the `execvpe()` function in `unistd.h` which redirects the call to `execve()`.
 */
int execvpe_hook(bool hook, const char *name, char *const *argv, char *const *envp);

/**
 * Hook for the `fexecve()` function in `unistd.h` which redirects the call to `execve()`.
 */
